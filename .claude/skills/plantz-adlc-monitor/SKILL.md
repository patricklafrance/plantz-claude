...
**Step 0 — Discover the PR.** Look up the open PR for the branch via `gh pr list --head <branch> --state open --json number --jq '.[0].number'`. If no PR is found, return failure.

The monitoring loop has a **rolling 30-minute timeout**. The timer resets each time a fix is pushed. Poll every 60 seconds.
...
2. If Phase 1 passes → update the CI Validation comment showing Phase 1 green and Chromatic pending. Add the `run chromatic` label to the PR. Proceed to Phase 2.
3. **Phase 2 — Monitor Chromatic.** Monitor the `Chromatic` workflow. Same polling pattern.
4. If Phase 2 passes → post the CI Validation comment showing all green. Return success.
5. **On failure (Phase 1 or Phase 2)** → follow the Fix procedure (below). After pushing the fix, **reset the 30-minute timer** and **re-monitor from Phase 1**. When Phase 1 is green, add the `run chromatic` label and proceed to Phase 2. This ensures Chromatic always runs against the latest pushed code — the label is consumed by each Chromatic run, so it must be re-added after every fix push.
6. **On timeout** (30 minutes since last push with no fix attempt) → post the CI Validation comment showing which workflows are still running. Return failure.
...
3. **Fix the code.** Read the plan file — the architect's inline `**Contract**:` lines and `**ARCHITECT CONSTRAINT**:` blocks in File Changes are authoritative for signatures, interfaces, and component boundaries. Do not change these — note the conflict and return failure instead. Edit the source files to address the failure. Stage only the files you modified.
4. **Validate locally.** Run `pnpm lint`, `pnpm sizecheck`, and `pnpm test` from the workspace root. If any fails, fix the issue (this does not consume an additional fix attempt — it is part of the current attempt). For bundle-size failures, follow `agent-docs/references/bundle-size-budget.md` — optimize code before considering a budget increase.
