...
**Only list module package names (`@modules/*`).** Never add shared packages (`@packages/*`) — Turborepo's transitive dependency detection already handles them. If `@packages/core-plants` changes, Turborepo marks the modules that depend on it as affected, which then triggers the storybook.
