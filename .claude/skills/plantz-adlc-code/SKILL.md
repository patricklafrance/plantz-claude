...
| `iteration`    | Current code-test iteration number (starts at 1). This is the iteration the agent will **write** to (`changes-[iteration].md`).                 |
...
6. **Start the dev server or Storybook before implementing.** Run `pnpm dev-host` for app routes, or `pnpm dev-storybook` for stories. If the plan affects both routes and stories, start `pnpm dev-host` (it serves the full app including module routes). Switch to `pnpm dev-storybook` when implementing story files that don't need the full app context. Use agent-browser to see what you're building as you go — this is not optional (see Hard Constraints). If the dev server or Storybook fails to start within 90 seconds, stop implementation — do not produce `changes-[iteration].md`. The orchestrator will detect the missing file and follow failure handling. Implement in dependency order: shared packages (`@packages/*`) first since modules consume them, then complete each affected module end-to-end (data layer → components → integration) before moving to the next module. Verify each module works in the browser before starting the next. Follow all technology rules from the `agent-docs/references/` files read in step 1.
...
- **Subagent A MUST use agent-browser during implementation.** Writing UI code without looking at it in the browser produces lower-quality output and wastes test-fix iterations. Use it to see what you're building — this is not optional, not advisory, and not something to skip to save context.
