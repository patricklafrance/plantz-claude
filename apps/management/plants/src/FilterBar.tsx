...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string | null; onChange: (value: string | null) => void; options: readonly { id: string; label: string }[] }) {
    return (
        <div className="flex items-center gap-2">
            ...
            <Select value={value ?? ""} onValueChange={(val: string | null) => onChange(!val || val === "" ? null : val)}>
    ...
    return (
        <div className="flex flex-wrap items-center gap-4 rounded-lg border border-border bg-muted/30 p-3">
            <div className="flex items-center gap-2">
                ...
                <Input className="h-7 w-36 text-xs" placeholder="Filter by name..." value={filters.name} onChange={(e) => onFilterChange("name", e.target.value)} />
            </div>
            ...
            <div className="flex items-center gap-2">
                <Label className="text-xs text-muted-foreground whitespace-nowrap">Mist leaves</Label>
                <Switch size="sm" checked={filters.mistLeaves === true} onCheckedChange={(checked: boolean) => onFilterChange("mistLeaves", checked ? true : null)} />
            </div>
            <div className="flex items-center gap-2">
                <Label className="text-xs text-muted-foreground whitespace-nowrap">Soil type</Label>
                <Input className="h-7 w-28 text-xs" placeholder="Filter..." value={filters.soilType} onChange={(e) => onFilterChange("soilType", e.target.value)} />
            </div>
            ...
            <div className="flex items-center gap-2">
                <Label className="text-xs text-muted-foreground whitespace-nowrap">Due for watering</Label>
                <Switch size="sm" checked={filters.dueForWatering} onCheckedChange={(checked: boolean) => onFilterChange("dueForWatering", checked)} />
            </div>
            {hasActiveFilters && (
