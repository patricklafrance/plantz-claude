...
Read-only audit of `CLAUDE.md` and `agent-docs/` against the live codebase. Report findings — never fix them during the audit.
...
- Never modify any file during an audit. The audit is read-only.
- Never create PRs or commits from audit findings. Report only.
