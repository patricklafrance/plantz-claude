...
`smoke-tests.yml` runs on PRs to `main`. It uses `claude-code-action` to load the `plantz-smoke-tests` skill in standalone mode (no `affected-pages`). Each app's dev server is started, verified in a browser, then stopped with port cleanup. Results are posted as a PR comment. 30-minute timeout with concurrency group (`cancel-in-progress`).
