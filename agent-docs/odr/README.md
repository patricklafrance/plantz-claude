...
4. Keep **Consequences** focused on impacts: capabilities gained, constraints introduced, trade-offs accepted. Do not include procedural guidance ("when X happens, do Y") or configuration how-tos — those belong in `agent-docs/references/`. ODRs record _why_ a decision was made; reference docs show _how_ to work with it.
