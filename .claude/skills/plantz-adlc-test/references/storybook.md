...
- **Chromatic modes:** All domain story files (pages, components, dialogs) must set `parameters: { chromatic: { modes: { "light mobile": { globals: { theme: "light" }, viewport: 375 }, "light tablet": { globals: { theme: "light" }, viewport: 768 }, "light desktop": { globals: { theme: "light" }, viewport: 1280 }, "dark mobile": { globals: { theme: "dark" }, viewport: 375 }, "dark tablet": { globals: { theme: "dark" }, viewport: 768 }, "dark desktop": { globals: { theme: "dark" }, viewport: 1280 } } } }` in `meta` so Chromatic captures mobile, tablet, and desktop snapshots in both light and dark themes. Do NOT use legacy `chromatic.viewports` — it conflicts with the global `chromatic.modes` in preview.tsx. This does not apply to `packages/components/` primitive stories (they use the preview-level light/dark modes only).
